...
extends Item {
	...
	public ItemApple()
	{
		setItemID(ID_APPLE);
		setHand(BOTH_HANDS);
		setName(TextLoader.getText("item_apple"));
	}
	...
	{
		if(inCam) screen.render(Sprites.items.getSprite(0, SPRITES_FOOD_Y, 16, 16), x, y, size, size, rot, alpha);
		else screen.renderGUI(Sprites.items.getSprite(0, SPRITES_FOOD_Y, 16, 16), x, y, size, size, rot, alpha);
	}
	
	@Override
	public void renderInHand(Screen screen, Player player, int hand)
	{
		screen.render(Sprites.appleInHand.getSprite(player.sx, player.sy, 64, 64), player.getX(), player.getY(), 256, 256, 0, 1);
	}
