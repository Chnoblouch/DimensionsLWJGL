...
public class CookingPotCraftingMenu 
extends InventoryMenu {
	
	private ArrayList<CraftingButton> buttons = new SafeArrayList<>();
		
	public CookingPotCraftingMenu(Inventory inventory)
	{
		super(inventory);
		setMenuID(MENU_COOKING_POT_CRAFTING);
		
		int x = -1;
		int y = 0;
		
		for(int i = 0; i < CraftingRecipes.getCookingRecipes().size(); i++)
		{
			x ++;
			if(x >= 3) 
			{
			...
			buttons.add(new CraftingButton(this, -300 + (x * 85), inventory.firstSlotY + (y * 85), 
										   CraftingRecipes.getCookingRecipes().get(i)));
		}
	}
