...
public class OvenCraftingMenu 
extends InventoryMenu {
	
	private ArrayList<CraftingButton> buttons = new SafeArrayList<>();
		
	public OvenCraftingMenu(Inventory inventory)
	{
		super(inventory);
		setMenuID(MENU_OVEN_CRAFTING);
		
		int x = -1;
		int y = 0;
		
		for(int i = 0; i < CraftingRecipes.getOvenRecipes().size(); i++)
		{
			x ++;
			if(x >= 3) 
			{
			...
			buttons.add(new CraftingButton(this, -300 + (x * 85), inventory.firstSlotY + (y * 85),
										   CraftingRecipes.getOvenRecipes().get(i)));
		}
	}
